...
mypoints <- read_sf("query/sites_points.geojson") %>% select(-cell_id) %>%
  # keep ONLY points located squarely within Boston
  st_join(read_sf("shapes/boston.geojson") %>% select(geometry), left = FALSE)
...
# Load data
tab <- read_csv("data/figure_rates.csv") %>%
  filter(figure == 0) %>%
...
mypoints <- read_sf("query/sites_points.geojson") %>% select(-cell_id) %>%
  ...
  mutate(ncells = n()) %>%
  mutate(type = type %>% recode_factor(
  "Community Space" =   "Community Spaces<br>(n = 189)",
  "Places of Worship" = "Places of Worship<br>(n = 208)",
  "Social Businesses" = "Social Businesses<br>(n = 195)",
  "Parks" = "Parks<br>(n = 451)")) %>%
  ungroup()

# How many are in study zone?
#mypoints %>% 
#  as_tibble() %>%
#  select(zone) %>%
#  .$zone %>% table()
